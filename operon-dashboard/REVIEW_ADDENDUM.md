...
## Checks added from 2026-04-27 Phase 5 external review

These five checks were filed against five real findings in the 2026-04-27 external review of the Phase 5 cleanup. The internal `/review-full` pass run that day missed all five — each is a category my review-skill execution didn't probe. Numbering continues from the highest existing IDs in each category.

### D-P10. Every Postgres function in `supabase/migrations/` revokes EXECUTE from PUBLIC + anon + authenticated
**What:** Every `CREATE OR REPLACE FUNCTION` in `supabase/migrations/` must be paired with `REVOKE EXECUTE ON FUNCTION <name>(...) FROM PUBLIC, anon, authenticated;` and `GRANT EXECUTE ON FUNCTION <name>(...) TO service_role;`. Application calls happen via `createServerSupabase()` (server-only, see [`lib/supabase/server.ts`](../lib/supabase/server.ts)) which uses the service-role key; anon-key callers (the browser, via `getSupabaseBrowser()` in [`lib/supabase/browser.ts`](../lib/supabase/browser.ts)) must NOT be able to invoke any RPC that mutates state or reads sensitive data.
**Why:** Supabase's PostgREST exposes every function in the `public` schema as a callable HTTP endpoint. Default Postgres grants `EXECUTE` to `PUBLIC`. Combined with the publicly-shipped `NEXT_PUBLIC_SUPABASE_ANON_KEY`, every undecorated RPC is reachable from any browser unless explicitly revoked. RLS is intentionally disabled (D-P7), so a callable RPC can read/write any table — there is no second line of defense. A 2026-04-27 external review surfaced this against migration 026's voucher-checkout RPCs (`reserve_node_purchase`, `complete_reservation`, `mark_reservation_submitted`, `mark_reservation_failed`, `expire_old_reservations`) — any of which a browser-side attacker could call directly to lock inventory or mark reservations completed without an on-chain purchase.
**Check:** For every new migration that creates or replaces a function, grep the file for `REVOKE EXECUTE`. If absent, blocking. For audit of existing migrations, grep `supabase/migrations/` for `CREATE OR REPLACE FUNCTION` and cross-check each against a `REVOKE EXECUTE` (or a documented exception). Callers that need the function must use `createServerSupabase()` (service role).
**Severity:** Blocking.
**Source:** 2026-04-27 external review of Phase 5 cleanup.

### A-P7. Inter-layer numeric bounds match the most restrictive consumer
**What:** When the same value (quantity, TTL, discount BPS, byte-array length, etc.) is bounded at multiple layers — DB CHECK constraint, Postgres RPC validation, Next.js API route validation, and smart contract `require` — the bounds at every layer must equal the most restrictive layer. A SQL CHECK that allows values the contract will reject creates a wedge: a request that slips past the API layer reaches the DB, gets a row written, then reverts on-chain, leaving the row in a quasi-state that may hold inventory or invariants the rest of the system thinks are committed.
**Why:** 2026-04-27 review found `026_sale_reservations.sql` allowed `quantity <= 1000` while `NodeSale.sol` caps at `MAX_BATCH_SIZE = 100`. The API route blocks `>100` so it didn't matter today, but combined with D-P10 (public RPC exposure) any browser attacker could create reservations the contract refuses, leaking inventory. Defense in depth requires every layer to enforce the same cap independently.
**Check:** For every numeric field that crosses (DB → RPC → API route → contract), enumerate the bound at each layer. Any layer with a wider bound than the strictest is a wedge surface and must be tightened. Common fields: tier id, quantity, TTL seconds, discount bps, batch size. The contract's `require` is canonical for fields it consumes.
**Severity:** Required (defense-in-depth; becomes Blocking if upstream gates can be bypassed, e.g. when D-P10 is unfixed).
**Source:** 2026-04-27 external review of Phase 5 cleanup.

### R-P6. Webhook event → DB-state convergence is one atomic RPC with full field verification
**What:** When an on-chain event triggers DB writes that span multiple invariants (commission rows, reservation completion, tier counter, partner credited_amount), all writes happen inside one Postgres function that **first re-verifies every event field against the prior DB state** (e.g. the reservation row's `buyer_wallet`/`tier`/`quantity`/`token` vs. the corresponding fields parsed from the on-chain log). Splitting the commission-write from the reservation/inventory-write across two separate RPC calls in `processPurchaseEvent` creates a partial-success window the cron retry path can mask but cannot prevent.
**Why:** D-P3 already mandates atomicity for the commission flow. The 2026-04-27 review found this discipline was abandoned at the boundary of the new voucher-checkout layer: `lib/webhooks/process-event.ts` calls `processReferralAttribution` then `complete_reservation` in sequence, and `complete_reservation` doesn't verify the event fields against the reservation row at all. A bug in voucher signing — or, if D-P10 is unfixed, a direct browser call to `complete_reservation` — could mark a mismatched reservation completed AFTER commissions were already credited.
**Check:** For every webhook handler that calls multiple Supabase RPCs in sequence, ask: "if step N succeeds and step N+1 fails, what's the system state?" If the answer is "partially correct, recoverable only via retry queue", the operations should collapse into one RPC. The unifying RPC must read the prior-state row, lock it (`FOR UPDATE`), compare every event field to the row, then write. Mismatch → return error envelope without writing anything.
**Severity:** Required.
**Source:** 2026-04-27 external review of Phase 5 cleanup. Echoes D-P3 with extension for cross-table convergence.

### O-P7. Threat-model claims in DECISIONS.md are grep-verified against the code
**What:** Any `DECISIONS.md` entry that claims a numeric or behavioural bound on adversary capability ("compromise of X is bounded by Y", "leaked key cannot do Z because of W") must reference identifiers that exist in the code being decided about. A claim that "leaked voucher signer compromise is bounded by `MAX_DISCOUNT_BPS`" requires `MAX_DISCOUNT_BPS` to be a real, enforced constant in the contract — not just a name in prose.
**Why:** 2026-04-27 review found `DECISIONS.md` D31 claimed leaked-signer compromise was bounded by `MAX_DISCOUNT_BPS × tierMinPrice × MAX_BATCH_SIZE`. The contract has no `MAX_DISCOUNT_BPS` — only a `<= 10000` (100%) sanity check, which is no meaningful bound. The doc statement was load-bearing for the security model but unverified against code; a reader trusting the doc would underestimate the true compromise impact.
**Check:** When a review touches `DECISIONS.md` or `REVIEW_ADDENDUM.md`, every constant name used in a security or correctness bound must grep-match a real identifier in the code path it describes. If the doc names a constant that does not exist in code, either add the constant (preferred — the doc is right and the code needs to catch up) or correct the doc. Never leave the doc as the only source of a bound.
**Severity:** Required (writeable as Blocking when the bound is the entire security argument).
**Source:** 2026-04-27 external review of Phase 5 cleanup.

### O-P8. Refactor sweep — every caller of the old path is updated when a new path becomes canonical
**What:** When a refactor introduces a new "blessed" RPC, function, or pattern that supersedes an existing one, the orphan-inverse part of Pass 5 must be inverted: grep for every call site of the OLD function and verify each is either updated to the new path or has a documented reason to stay on the old. Catching one orphan (zero callers of new function) is the easy half; catching the inverse — every still-using-it site of the legacy path — requires an explicit sweep.
**Why:** 2026-04-27 review found that F66 made `complete_reservation` canonical in `process-event.ts` and `cron/reconcile/route.ts`, but `app/api/admin/events/replay/route.ts:109` and `app/api/dev/replay-failed-events/route.ts:70` still called `increment_tier_sold` (legacy). The internal review's Pass 5 caught `mark_reservation_failed` had zero callers, but missed the dual case: legacy `increment_tier_sold` had two callers it shouldn't have. Same root cause (inventory leak via reservation rows stuck in `'submitted'`), three sites instead of one.
