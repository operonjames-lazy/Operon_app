...
**What:** Every `CREATE OR REPLACE FUNCTION` in `supabase/migrations/` must be paired with `REVOKE EXECUTE ON FUNCTION <name>(...) FROM PUBLIC, anon, authenticated;` and `GRANT EXECUTE ON FUNCTION <name>(...) TO service_role;`. Application calls happen via `createServerSupabase()` which uses the service-role key; anon-key callers (the browser, via `getSupabaseBrowser()` in [`lib/supabase.ts:34`](../lib/supabase.ts)) must NOT be able to invoke any RPC that mutates state or reads sensitive data.
