...
| DB14 | Partner status enforcement on commission RPC | ✅ Done | Migration 021. `CREATE OR REPLACE` of `process_purchase_and_commissions` so the chain walk reads `epp_partners.status` and skips uplines that are not `'active'`. Closes the silent-decorative-suspend bug surfaced by /grill — before this, `/api/admin/partners/status` set the column but the RPC ignored it, so suspended partners kept earning on every new purchase. Historical `referral_purchases` rows untouched. |
| DB15 | Overview "today" UTC bucket + milestones RPC | ✅ Done | Migration 022. (1) `admin_overview_stats.revenue.today` re-keyed from rolling-24h to UTC-date so the KPI tile matches the chart. (2) New `admin_milestones_pending()` RPC + `/api/admin/payouts/milestones` rewrite (the route was missed in 020's D-9 sweep). Also fixed a 100×-too-high threshold/bonus bug in the route's TS table — numeric-separator literal `1_000_000_00` was parsing as 100,000,000 cents, not the $10,000 the comment claimed. RPC uses migration 010's authoritative thresholds. |
| DB16 | Partner-leaderboard / pipeline / user-search RPCs (D-P9 sweep completion) + cron advisory lock | ✅ Done | Migration 023. New STABLE RPCs `admin_partner_leaderboard`, `admin_partner_pipeline`, `admin_user_purchase_counts` close the last D-P9 violations 020 missed — `partners/list`, `partners/pipeline`, `users/search` had been aggregating in JS over unbounded SELECTs. Same migration adds `try_reconcile_lock()` (`pg_try_advisory_lock` wrapper) called at top of `/api/cron/reconcile` so concurrent runs don't race on signer nonces, plus 3 announcement killswitch keys (`admin.announcements.{create,toggle,delete}`) now wired in the route handlers. The earlier `partners/pipeline` route had hardcoded thresholds with JS numeric-separator literals — same bug class as DB15 — fixed by deriving from `lib/commission.ts`. |
| F30 | Pending-attribution banner on /nodes | ✅ Done | After a confirmed-on-chain purchase, sale page writes `operon_pending_attribution` to localStorage (tx hash + chain + wallet + ts, 15-min TTL). `/nodes` shows a pending banner with explorer link until the tx hash appears in the ingested `purchases` rows or TTL expires. Closes the "Purchase Complete!" + "No Nodes Yet" UX gap visible on `pnpm dev` (no webhook) and during real webhook delivery delays. Localised to all 6 languages. Pairs with the now-honest "Live updates" badge on `/sale` (only shown when Realtime is genuinely offline). |
...
| F66 | Phase 5 cleanup — strip dead referral-sync surface | ✅ Done | Stripped `REFERRAL_ADMIN_ABI` + `TIER_ADMIN_ABI` + `getReferralAdminContract` + `getTierAdminContract` from `lib/admin-signer.ts`. Deleted `lib/referrals/sync-on-chain.ts`, migration `024_referral_code_owner_wallet.sql`, `/api/admin/sale/tier-active`, `/api/dev/drain-referrals`, and (collateral) `/api/admin/referrals/remove`. Removed `enqueueReferralSync` calls from `/api/auth/wallet`, the referral-sync drain block from `/api/cron/reconcile`, the TierActions UI from `(admin)/admin/sale/page.tsx`, and the drain call from `scripts/dev-indexer.mjs`. Updated `NodePurchased` ABI in `lib/webhooks/process-event.ts`, `app/api/cron/reconcile/route.ts`, `scripts/dev-indexer.mjs`, `scripts/test-webhooks.mjs` to include `uint256 indexed tier` + `bytes32 indexed reservationId`. `processPurchaseEvent` now calls `complete_reservation` (links tx → reservation, bumps tier counter, auto-advances tier) in place of the old `increment_tier_sold`. Reservation lookup failures park as `failed_events.kind='reservation_link_error'`. Build green, 53 contract tests passing, typecheck clean. |
| F66.1 | Phase 5 follow-up — drop `referral_code_chain_state` table | ✅ Done | Migration `027_drop_referral_code_chain_state.sql` drops the table (cascading the `'revoked'` CHECK from migration 018) and clears the orphan `admin.sale.tier-active` / `admin.referrals.remove` / `admin.referrals.reset` killswitch rows from migration 019's seed. Deleted `/api/admin/referrals/reset` route, the `syncQueue` reads from `/api/admin/health` + the matching admin-UI card + the `Row` helper + the `Badge` import, and the now-stale `e2e/full-chain/referral-sync.spec.ts`. **Drift discovery:** `scripts/verify-pending-migrations.mjs` revealed 025/026/027 were all unapplied on the live DB — Phase 3+4 voucher code had been live in the repo while the matching DB layer wasn't. All three applied in order on 2026-04-27. Build green, typecheck clean, 53/53 contract tests passing. |
