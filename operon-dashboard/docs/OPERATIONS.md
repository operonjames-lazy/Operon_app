...
| `039_reservation_one_per_wallet_per_tier.sql` | R9 security audit follow-up — one active reservation per `(wallet, tier)` across BOTH chains. Mig 038's existing-reservation lookup filtered by `chain = p_chain`, leaving open a cross-chain hoarding hole: one wallet could simultaneously hold reservations on Arbitrum AND BSC for the same tier, locking 2× per-wallet inventory at sellout boundaries. `CREATE OR REPLACE` of `reserve_node_purchase` drops `chain = p_chain` from the existing-row filter; instead, `chain` joins the exact-match comparison so a same-tier-different-chain request is correctly classified as `existing_active_reservation` (409) rather than silently creating a second reservation. The `sale_tiers FOR UPDATE` lock at the active-tier read serializes concurrent attempts so two simultaneous Arb+BSC requests from the same wallet cannot both pass the existing-reservation check. |
