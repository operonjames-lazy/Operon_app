...
* **Test 7 (NEW) — multi-level referral chain.** A → B → C, Wallet C buys, verify all three uplines paid at descending rates (L1 = 10% / 1000bps, L2 = 3% / 300bps per `lib/commission.ts COMMUNITY_COMMISSION_RATES`). The post-mig-31 lowercase-URL signup-time normalize is verified internally (synthesized data + DB case-sensitivity proof) rather than via a tester click-through; no fresh wallet required.
