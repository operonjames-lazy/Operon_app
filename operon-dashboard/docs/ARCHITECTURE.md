...
| `/api/sale/validate-code` | POST | Preview-only — drives the green/red badge as the user types. Backed by `lib/referrals/validate.ts`; for authenticated callers with a bound referrer, also previews the strict `(referrer_id, code_used)` match enforced by `lib/referrals/checkout-attribution.ts` so the green ✓ never lies about a code Reserve will then reject (R11-03 / A-P7) |
...
1. Buyer hits `POST /api/sale/reserve` → backend validates code via `lib/referrals/validate.ts` (self-referral block) → runs `lib/referrals/checkout-attribution.ts::ensureCheckoutCodeAttribution` to enforce that the voucher's `(referrer_id, code_used)` matches the buyer's bound `referrals` row exactly (NULL → INSERT bind, mismatch → 409 `referrer_locked`, race → 23505 re-read; see D36 / A-P7 / R11-03 closure) → calls `reserve_node_purchase` RPC (atomic `FOR UPDATE` on the active tier row, validates qty against `total_supply - total_sold - active_reservations`) → inserts `sale_reservations` row → signs EIP-712 voucher with `VOUCHER_SIGNER_PRIVATE_KEY` and returns `{reservationId, voucher, signature, expiresAt}`
