...
**如果你测过 2026-04-18 的那一版:** 感谢回来。搭建上的差别很小——只多了一个迁移文件(`018_revoked_referral_status.sql`),在 `017` 之后执行。没有新的环境变量,没有新的依赖,合约也不需要重新部署。趁着这次,顺手多测三件事:

1. **管理员撤销推荐码。** 先用一个有效推荐码完成一笔购买,再让操作人员通过管理接口撤销这个码。然后试着再用同一个码去买——应该退回到"无折扣",而且这个码不应该在 5 分钟内自己在链上悄悄恢复。
2. **跨钱包的挂起交易恢复。** 发起一笔购买,交易进行中关掉标签页,换到 MetaMask 里另一个钱包,然后重新打开销售页。页面上**不应该**在第二个钱包下出现那条黄色的"挂起交易"提示条。如果出现了,立刻上报。
3. **实时重连。** 停留在销售页时,用浏览器开发者工具把网络切到 Offline 10 秒,再切回 Online。层级 / 状态应该重新对齐,而不是停留在断线时看到的数据。
...
3. 在文件管理器里打开 `operon-dashboard/supabase/migrations` 文件夹。注意:**`002_seed_data.sql` 已经移到 `supabase/testnet-only/`,不在这个文件夹里**——它会在 3.7.1 单独应用。
4. 用文本编辑器打开 `001_initial_schema.sql`。全选,复制,粘贴到 Supabase SQL Editor。点击 **Run**。
5. 等到显示 **Success**。
6. 清空编辑器。按**编号顺序**对剩下的文件依次重复(**跳过 002**,它在 3.7.1 应用):003、004、005、006、008、009、010、011、012、013、014、015、016、017、018。**没有 007 —— 直接跳过。**

任何一个文件报错就停下,联系操作人员。

说明：
- `supabase/testnet-only/002_seed_data.sql`(3.7.1 应用)预先写入了一批 EPP 邀请码,你在 Test 5 里可以直接用这些。它还会插入几行演示数据(一个虚构的 "David Kim" EPP 合作伙伴、两条历史购买记录),纯粹是为了仪表板截图。**绝对不要在主网应用这个文件**——它原本会污染 sale_tiers 让 tier 1 显示成「已售完」。R8 已修掉那两行,但仍属于测试网专用。
...
- `017_guard_tier_reset.sql` 是 `014` 的补强:如果数据库里已经有真实购买记录,就跳过 `014` 会做的层级计数清零——让你在测试途中可以安全地重跑迁移清单,不会破坏计数。必须排在 `016` 之后。**不要单独重跑 `014`**——它的层级计数清零是无条件的。如果需要重置状态,请按 `014 → 015 → 016 → 017` 完整顺序重跑,这样 `017` 中的保护层才会保护真实的购买记录。
- `018_revoked_referral_status.sql` 把 `referral_code_chain_state` 的状态列扩展到允许 `'revoked'` 作为终止态,这样管理员撤销推荐码的动作就不会在 5 分钟内被 drain 循环静默撤回。在 `017` 之后执行。
