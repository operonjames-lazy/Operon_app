...
const en: EppLangPack = {
  ...
  sec: [
    ...
    { n: '4', t: 'Commissions and Payment', b: 'Pre-TGE commissions are paid biweekly in USDC to the Partner\'s registered payout wallet, subject to fraud review. Post-TGE commissions are paid in $OPRN from the Referral and Distribution Pool. Commission rates are provided in the programme documentation sent upon acceptance.' },
...
const tc: EppLangPack = {
  ...
  sec: [
    ...
    { n: '4', t: '佣金與付款', b: 'TGE 前佣金每兩週以 USDC 支付至合作夥伴登記的收款錢包，須通過反詐審核。TGE 後佣金以 $OPRN 從推薦與分配池中支付。佣金費率將在接受後的計畫文件中提供。' },
...
const sc: EppLangPack = {
  ...
  sec: [
    ...
    { n: '4', t: '佣金与支付', b: 'TGE 前佣金每两周以 USDC 支付至合作伙伴登记的收款钱包，须通过反欺诈审核。TGE 后佣金以 $OPRN 从推荐与分配池中支付。佣金费率将在接受后的计划文件中提供。' },
...
const ko: EppLangPack = {
  ...
  sec: [
    ...
    { n: '4', t: '커미션 및 지급', b: 'TGE 전 커미션은 격주로 파트너가 등록한 지급 지갑에 USDC로 지급되며, 사기 검토를 거칩니다. TGE 후 커미션은 추천 및 분배 풀에서 $OPRN으로 지급됩니다. 커미션 비율은 수락 후 제공되는 프로그램 문서에 포함됩니다.' },
...
const vi: EppLangPack = {
  ...
  sec: [
    ...
    { n: '4', t: 'Hoa Hồng và Thanh Toán', b: 'Hoa hồng trước TGE được thanh toán hai tuần một lần bằng USDC vào ví thanh toán đã đăng ký của Đối Tác, tuỳ thuộc vào quá trình kiểm tra gian lận. Hoa hồng sau TGE được thanh toán bằng $OPRN từ Quỹ Giới Thiệu và Phân Phối. Tỷ lệ hoa hồng được cung cấp trong tài liệu chương trình sau khi chấp nhận.' },
...
const th: EppLangPack = {
  ...
  sec: [
    ...
    { n: '4', t: 'ค่าคอมมิชชันและการชำระเงิน', b: 'ค่าคอมมิชชันก่อน TGE จะชำระเป็น USDC ทุกสองสัปดาห์ไปยังกระเป๋าเงินรับชำระที่พาร์ทเนอร์ลงทะเบียนไว้ โดยต้องผ่านการตรวจสอบการฉ้อโกง ค่าคอมมิชชันหลัง TGE จะชำระเป็น $OPRN จากกองทุนการแนะนำและการแจกจ่าย อัตราค่าคอมมิชชันจะระบุในเอกสารโครงการที่จัดส่งหลังการตอบรับ' },
    ...
    { n: '6', t: 'หน้าที่ของพันธมิตร', b: 'พันธมิตรต้องอธิบายโหนดว่าเป็นใบอนุญาตเข้าร่วมเครือข่าย ห้ามสื่อสารถึงผลตอบแทนที่รับประกัน ห้ามเสนอส่วนที่เกี่ยวกับโทเคนแก่บุคคลสัญชาติสหรัฐอเมริกา เงื่อนไขโครงการเป็นข้อมูลลับ สื่อการตลาดทั้งหมดต้องใช้แบรนด์ที่ได้รับอนุมัติแล้วเท่านั้น' },
