...
**和第 1 轮有什么不同：** 这是第二轮用户测试。第一轮(2026-04-14)暴露出 14 个 bug,全部已经修复,代码也经过了端到端复审,本次交付反映的就是修复后的状态。Part 3 的搭建步骤里有两项是新的——**即便你参加过第 1 轮,也请仔细读完 Part 3**。本指南末尾的 Part 9 列出了已知的遗留事项——这些是有意推迟到以后处理的项目,不是希望你上报的 bug。
